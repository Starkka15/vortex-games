...
//#endregion
//#region extensions/games/game-baldursgate3/pakParser.ts
/**
...
function decompressLZ4Block(src, uncompressedSize) {
	...
	while (sIdx < src.length && dIdx < uncompressedSize) {
		...
		for (let i = 0; i < matchLen; i++) dst[dIdx++] = dst[matchPos++];
	}
	return dst;
}
const LSPK_MAGIC = 1263555404;
const ENTRY_SIZE_V15 = 296;
const ENTRY_SIZE_V18 = 272;
const NAME_SIZE = 256;
var CompressionMethod = /* @__PURE__ */ function(CompressionMethod) {
	CompressionMethod[CompressionMethod["None"] = 0] = "None";
	CompressionMethod[CompressionMethod["Zlib"] = 1] = "Zlib";
	CompressionMethod[CompressionMethod["LZ4"] = 2] = "LZ4";
	CompressionMethod[CompressionMethod["Zstd"] = 3] = "Zstd";
	return CompressionMethod;
}(CompressionMethod || {});
async function readBytes(fd, offset, length) {
	const buf = Buffer.alloc(length);
	await fd.read(buf, 0, length, offset);
	return buf;
}
function nullTermString(buf) {
	let end = buf.indexOf(0);
	if (end === -1) end = buf.length;
	return buf.toString("utf8", 0, end);
}
async function parseHeader(fd) {
	const head = await readBytes(fd, 0, 8);
	const magic = head.readUInt32LE(0);
	if (magic !== LSPK_MAGIC) throw new Error(`Not a valid BG3 PAK: bad magic 0x${magic.toString(16)}`);
	const version = head.readUInt32LE(4);
	if (version !== 15 && version !== 16 && version !== 18) throw new Error(`Unsupported PAK version: v${version}`);
	const rest = await readBytes(fd, 8, version === 15 ? 30 : 32);
	return {
		version,
...
async function parseFileList(fd, header) {
	...
	const data = decompressLZ4Block(compressed, numFiles * entrySize);
	const entries = [];
	for (let i = 0; i < numFiles; i++) {
		const off = i * entrySize;
		const name = nullTermString(data.subarray(off, off + NAME_SIZE));
		let entry;
		if (header.version === 18) {
			...
			entry = {
				...
				compressionMethod: data[off + 263] & 15,
			...
			compressionMethod: data.readUInt32LE(off + 284) & 15
		};
		entries.push(entry);
	}
	return entries;
}
async function extractFileData(fd, entry) {
	const rawData = await readBytes(fd, Number(entry.offsetInFile), entry.sizeOnDisk);
	switch (entry.compressionMethod) {
		case CompressionMethod.None: return rawData;
		case CompressionMethod.LZ4: return decompressLZ4Block(rawData, entry.uncompressedSize);
		case CompressionMethod.Zlib: return zlib.inflateSync(rawData);
		case CompressionMethod.Zstd: throw new Error(`Zstd compression not yet supported for "${entry.name}". Please report this PAK file.`);
		default: throw new Error(`Unknown compression method ${entry.compressionMethod} for "${entry.name}"`);
	}
}
/**
...
*/
async function listPakFiles(pakPath) {
	const fd = await fs.promises.open(pakPath, "r");
	try {
		return (await parseFileList(fd, await parseHeader(fd))).map((e) => `${e.name}\t${e.uncompressedSize}\t0`);
	} finally {
		await fd.close();
	}
}
...
*/
async function extractPakMeta(pakPath) {
	const fd = await fs.promises.open(pakPath, "r");
	try {
		const metaEntry = (await parseFileList(fd, await parseHeader(fd))).find((e) => path.basename(e.name).toLowerCase() === "meta.lsx");
		if (!metaEntry) return;
		return (0, xml2js.parseStringPromise)((await extractFileData(fd, metaEntry)).toString("utf8"));
	} finally {
		await fd.close();
	}
}
		...
		return vortex_api.util.withErrorContext("reading pak", fileName, () => {
			const func = async () => {
				try {
					...
					if (err.code !== "ENOENT") api.showErrorNotification("Failed to read pak file", err, {
						allowReport: false,
						message: fileName
					});
					return;
...
function InfoPanelWrap(props) {
	...
	const isLsLibInstalled = react.useCallback(() => {
		return process.platform === "linux" || getLatestLSLibMod(api) !== void 0;
	}, [api]);
...
function main(context) {
	...
	context.registerAction("mod-icons", 300, "settings", {}, "Re-install LSLib/Divine", () => {
		...
		return vortex_api.selectors.activeGameId(state) === GAME_ID && process.platform !== "linux";
	});
